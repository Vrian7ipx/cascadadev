...
{
  public function getInvoices($accountId, $branchId, $clientPublicId = false, $filter = false)
  {
      if (Utils::isAdmin())
      {
  ...
  public function save($publicId, $data)
  {
    ...
    $invoice->nit = trim($data['nit']);
    $invoice->name = trim($data['name']); 
    ...
    //   $invoiceItemCost = Utils::parseFloat($item->prettyCost);
    //   $invoiceItemQty = Utils::parseFloat($item->qty);
    //   $invoiceItemBoni = Utils::parseFloat($item->boni);
    //   $invoiceItemDesc = Utils::parseFloat($item->desc);
    //   $invoiceItemUnits = Utils::parseFloat($item->units);
    ...
    $invoice->subtotal = Utils::parseFloat($data['subtotal1']);
    $invoice->amount = Utils::parseFloat($data['amount1']);
    $invoice->fiscal = Utils::parseFloat($data['fiscal1']);
    $invoice->save();
    ...
    foreach ($data['invoice_items'] as $item) 
    {
      ...
      $invoiceItem->cost = Utils::parseFloat($item->cost/$item->units);
      $invoiceItem->qty = Utils::parseFloat($item->totalqty);
      $invoiceItem->boni = Utils::parseFloat($item->boni);
      $invoiceItem->desc = Utils::parseFloat($item->desc);
      $invoiceItem->line_total = Utils::parseFloat($item->totalqty*$item->cost/$item->units);
