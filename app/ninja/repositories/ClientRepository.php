...
class ClientRepository
{
	public function find($filter = null)
	{
    	$query = \DB::table('clients')
    				...
    				->select('clients.public_id','clients.id','clients.nit','clients.name','contacts.first_name','contacts.last_name','clients.balance','clients.last_login','clients.created_at','clients.work_phone','contacts.email','clients.currency_id');
    	...
    	if ($filter)
    	{
			...
			else
			{
	    		$query->where(function($query) use ($filter)
	            {
	            	$query->where('clients.nit', 'like', $filter.'%')
	...
	public function save($publicId, $data, $notify = true)
	{
		...
		if (isset($data['business_type_id'])) {
			$client->business_type_id = $data['business_type_id'] ? $data['business_type_id'] : null;
		}
