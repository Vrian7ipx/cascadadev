...
class AccountController extends \BaseController {
	...
	public function __construct(AccountRepository $accountRepo, UserMailer $userMailer, ContactMailer $contactMailer, TaxRateRepository $taxRateRepo)
	{
		parent::__construct();
	...
	public function showSection($section = ACCOUNT_DETAILS, $subSection = false)  
	{
		...
		else if ($section == ACCOUNT_GROUPS)
		{
			$data = [
				'account' => Auth::user()->account
			];
	...
	private function saveNotifications()
	{
		...
		Session::flash('message', trans('texts.updated_settings'));
		return Redirect::to('company/advanced_settings/notifications');
	}
	...
	private function saveDetails()
	{
		...
		else 
		{
			...
			$tax = Auth::user()->account->tax_rates()->first();
			$tax->name = 'ICE';
			$tax->rate = trim(Input::get('tax'));
			$tax->save();
