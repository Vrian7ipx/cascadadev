...
class Activity extends Eloquent
{
	...
	public static function emailInvoice($invitation)
	{
		...
		$activity->message = Utils::encodeActivity(Auth::check() ? Auth::user() : null, 'envió por correo', $invitation->invoice, $invitation->contact);
		$activity->balance = $client->balance;
		$activity->save();
	}
	...
	public static function updateInvoice($invoice)
	{
		if ($invoice->is_deleted && !$invoice->getOriginal('is_deleted'))
		{
			...
			$activity->message = Utils::encodeActivity(Auth::user(), 'borró', $invoice);
			$activity->balance = $invoice->client->balance;
	...
	public static function createPayment($payment)
	{
		...
		if ($payment->contact_id)
		{
			...
			$activity->message = Utils::encodeActivity($payment->invitation->contact, 'ingreso el pago' . $payment->getName());			
		}
		else
		{
			$activity = Activity::getBlank();
			$message = $payment->payment_type_id == PAYMENT_TYPE_CREDIT ? 'crédito aplicado' : 'pago ingresado ' . $payment->getName();
	...
	public static function updatePayment($payment)
	{
		if ($payment->is_deleted && !$payment->getOriginal('is_deleted'))
		{
			...
			$activity->message = Utils::encodeActivity(Auth::user(), 'borró el pago ' . $payment->getName());
			$activity->balance = $client->balance;
	...
	public static function archivePayment($payment)
	{
		...
		$activity->message = Utils::encodeActivity(Auth::user(), 'archivó el pago ' . $payment->getName());
		$activity->balance = $client->balance;
	...
	{
		$activity = Activity::getBlank();
		$activity->message = Utils::encodeActivity(Auth::user(), 'aplicó ' . Utils::formatMoney($credit->amount, $credit->client->currency_id) . ' credit');
	...
	public static function updateCredit($credit)
	{
		if ($credit->is_deleted && !$credit->getOriginal('is_deleted'))
		{
			...
			$activity->message = Utils::encodeActivity(Auth::user(), 'borró ' . Utils::formatMoney($credit->balance, $credit->client->currency_id) . ' credit');
			$activity->balance = $credit->client->balance;
			$activity->save();		
		}
		else
	...
	public static function archiveCredit($credit)
	{
		...
		$activity->message = Utils::encodeActivity(Auth::user(), 'archivó ' . Utils::formatMoney($credit->balance, $credit->client->currency_id) . ' credit');
		$activity->balance = $credit->client->balance;
		$activity->save();
	}
